...
{
    [Authorize(Roles = "Admin,Manager,Student,Teacher")]
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> RegisterStudent(RegisterStudentViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    await this.UserManager.AddToRoleAsync(user.Id, model.UserRole = "Student");
                    return RedirectToAction("Index", "Home");
                }
                AddErrors(result);
            }
        ...
        //
        // GET: /Account/Register_Teacher
        [AllowAnonymous]
        public ActionResult Register_Teacher()
        {
            ViewBag.Name = new SelectList(context.Roles.Where(u => !u.Name.Contains("Teacher")).ToList(), "Name", "Name");
            return View();
        }

        //
        // POST: /Account/Register_Teacher
        [HttpPost]
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
